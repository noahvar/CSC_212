...
bool Sujiko::solve(unsigned int cell){
    ...
    for(int row = 0; row < 3; row++)
    {
        for(int col = 0; col < 3; col++)
        {
            if(board[row][col] != 0)
            {
                missing.erase(std::remove(missing.begin(), missing.end(), board[row][col]));
            }
        }
    }
...
bool Sujiko::isValid()
{
    ...
    if(topLeft == topLeftCheck)
    {
        return true;
    }
    if(topRight == topRightCheck)
    {
        return true;
    }
    if(botLeft == botLeftCheck)
    {
        return true;
    }
    if(botRight == botRightCheck)
    {
        return true;
    }
